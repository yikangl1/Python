...
from datamodel.search.Yikangliu_datamodel import YikangliuLink
...
@Deleter(YikangliuLink)
@Getter(YikangliuLink)
class DeleteFrontierFrame(IApplication):
    ...
    def update(self):
        ...
        ls = self.frame.get(YikangliuLink)
        print "Found ", len(ls), " links to delete."
        for l in ls:
            self.frame.delete(YikangliuLink, l)
...
class Simulation(object):
    ...
    def __init__(self, address, port):
        ...
        objectless_connector = ObjectlessSpacetimeConnection(
            "ResetFrontier_Yikangliu".format(CrawlerFrame.app_id),
